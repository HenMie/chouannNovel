...
- [x] **撤销/重做功能**: 节点编辑操作的撤销（Ctrl+Z）和重做（Ctrl+Shift+Z/Ctrl+Y）支持。
  - 新版 `useWorkflowHistory` 采用命令模式，按操作粒度执行 undo/redo
  - 创建、删除、粘贴、重排序、配置修改等关键操作全部注册为命令
  - 工具栏添加撤销/重做按钮
...
### 引导与帮助 ✅
- [x] **新手引导 (Onboarding)**: 首次使用时的交互式引导教程。
  - 新增 `Tour` 组件系统（Spotlight 高亮 + Tooltip 步骤提示）
  - 新增 `tour-store` 管理引导状态（Zustand + localStorage 持久化）
  - 覆盖四大模块：首页、工作流编辑、设定库、AI 配置
  - 首次访问自动触发，侧边栏提供「开始引导」手动入口
- [x] **快捷键提示**: 全局快捷键一览表（可通过 `F1` 打开）。
  - 新增 `shortcuts.ts` 快捷键配置中心，按模块分组定义
  - 新增 `ShortcutsDialog` 弹窗，支持上下文感知过滤
  - 根据当前页面自动显示相关快捷键
...
### 交互优化
- [ ] **拖拽上传**: 支持拖拽文件导入项目或工作流。
- [x] **右键菜单**: 完善节点和列表项的右键上下文菜单。
  - 工作流节点支持右键菜单：配置、复制、粘贴、选中、删除
  - 设定库条目支持右键菜单：编辑、启用/禁用、删除
  - 项目工作流卡片支持右键菜单：打开、编辑、删除
...
### 错误处理 ✅
- [x] **Error Boundary**: 添加 React Error Boundary 捕获组件渲染错误，提供友好的错误提示页面。
  - 新增 `AppErrorBoundary` 组件包裹应用入口
  - 渲染异常时展示友好的错误卡片，支持"重试"和"刷新应用"
  - 异常自动记录到控制台，便于后续排查
- [x] **全局错误处理**: 统一的 API 错误处理和用户反馈机制。
  - 新增 `src/lib/errors.ts` 统一错误工具：`getErrorMessage`、`logError`、`handleAppError`
  - 新增 `GlobalErrorListener` 监听 `error` / `unhandledrejection` 事件
  - 页面与 Store 层全部改用 `handleAppError` / `logError` 替换分散的 `console.error + toast`
  - 保留原始 error 对象，便于后续接入日志系统

### 日志与监控
- [x] **日志系统**: 引入统一的日志管理工具，替换分散的 `console.log/error` 调用。
  - 通过 `logError` 函数集中记录，支持 context 和 payload 附加信息
- [ ] **性能监控**: 添加关键操作的性能埋点。
...
### 类型安全 ✅
- [x] **消除 @ts-ignore**: 修复所有 TypeScript 类型问题，移除 `@ts-ignore` 注释。
  - 移除 `Sidebar.tsx` 中的 2 处 `@ts-ignore`，使用类型断言替代
  - 修复测试文件中的无效 NodeType 类型引用
- [x] **严格类型检查**: 已启用 TypeScript 严格模式。
  - `tsconfig.json` 已配置 `"strict": true`
  - 新增 `sql-js.d.ts` 类型声明文件
  - 修复所有未使用导入警告（约 20 处）
  - 修复 `useWorkflowHistory` 泛型类型问题
...
*最后更新: 2025-12-05* (性能优化、核心功能增强、编辑器增强、错误处理、类型安全、引导与帮助已完成)
