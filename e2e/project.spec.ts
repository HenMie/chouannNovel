...
test.describe('项目管理流程', () => {
  ...
  test.describe('创建项目', () => {
    test('应该成功创建新项目', async ({ page }) => {
      ...
      // 等待表单页面加载
      await expect(page.locator('h1, h2, h3, h4').filter({ hasText: '创建新项目' })).toBeVisible()
  ...
  test.describe('编辑项目', () => {
    ...
    test('应该成功编辑项目信息', async ({ page }) => {
      ...
      // 点击更多菜单按钮
      const moreButton = projectCard.locator('button').filter({ has: page.locator('svg.lucide-more-vertical') })
      ...
      // 验证成功提示
      await expect(page.locator('[data-sonner-toast]')).toContainText('更新成功')
    })
  })
  ...
  test.describe('删除项目', () => {
    ...
    test('可以取消删除项目', async ({ page }) => {
      ...
      // 验证项目仍然存在
      await expect(page.locator('.group').filter({ hasText: projectName }).first()).toBeVisible()
    })
  })
  ...
  test.describe('项目导航', () => {
    ...
    test('点击项目卡片应该进入项目详情', async ({ page }) => {
      // 点击项目卡片
      await page.locator('.group').filter({ hasText: projectName }).first().click()
