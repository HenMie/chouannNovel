...
### 3.1 Token 用量与成本追踪 — 完全缺失

**现状**: AI SDK 响应中的 `usage` 字段（promptTokens, completionTokens, totalTokens）已被捕获 (`src/lib/ai/types.ts:57-61`)，但:
- 未保存到数据库
- 无 UI 展示
- 无成本计算
- 无用量统计/图表
- `executions` 和 `node_results` 表均无 token 相关字段

**建议**:
1. `node_results` 表增加 `token_usage` JSON 字段
2. 执行详情页展示各节点 token 消耗
3. 基于模型定价估算成本
4. 提供项目/全局维度的用量仪表盘

### 3.2 AI 请求重试机制 — 完全缺失

**现状**: 执行引擎中 AI 请求失败后直接标记节点为 failed，整个工作流停止 (`executor.ts:223-248`)。

**缺失**:
- 无指数退避重试（对 429 Rate Limit 等瞬时故障）
- 无备用提供商切换
- 无断路器模式
- 用户必须手动重新执行
...
用户无法复制整个项目（含工作流和设定），必须手动重建。`exportProject()` + `importProject()` 可作为 workaround，但操作路径过长。
...
### 4.1 暂停无法中断正在进行的 AI 流

**位置**: `executor.ts:571-605`

暂停信号仅在节点之间检查，不在 AI 流式输出过程中生效。如果一个 AI 节点正在生成长篇内容（如 10 分钟的响应），用户点击暂停后必须等待该节点完成才能真正暂停。

`AbortController` 已存在但仅用于取消，未用于暂停中断。
...
当模板中引用不存在的变量（如 `{{不存在的变量}}`）时，`interpolate()` 静默返回原始占位符，不发出警告。用户不知道为什么输出包含未解析的 `{{...}}`。
...
AI 节点配置中的 temperature 等参数需手动调整，缺少"创意模式 / 平衡模式 / 精确模式"等预设快捷选项。
...
右侧详情面板中，选中子设定时不显示完整层级路径（如 "角色 > 主角 > 外貌描述"），用户难以判断当前位置。
...
AI 节点配置中的 provider/model 选择仅在执行时验证。如果提供商未启用或 API Key 未填，用户直到运行工作流才会发现错误。

**建议**: 在节点配置面板中添加实时验证提示（如 "⚠ 该提供商未配置 API Key"）。

### 6.6 流式输出无停滞指示

当 AI 响应中断或网络延迟时（>30s 无新 chunk），UI 无任何提示。用户无法区分"正在思考"和"已断连"。
...
| # | 问题 | 影响 |
|---|------|------|
| 4 | Token 用量/成本追踪 | 用户无法控制 AI 开销 |
| 5 | AI 请求重试机制 | 网络波动直接终止工作流 |
| 6 | 暂停无法中断 AI 流 | 长响应期间暂停无效 |
| 7 | 变量引用静默失败 | 调试困难 |
| 8 | 项目复制 | 基础 CRUD 能力不完整 |
| 9 | 执行历史对比/重放 | 迭代优化困难 |
...
| 11 | 流式 Markdown 全量重渲染 | 长输出 UI 卡顿 |
| 12 | 设定库拖拽排序 | 层级调整操作繁琐 |
| 13 | 节点配置预验证 | 错误发现太晚 |
| 14 | 执行历史筛选/搜索 | 查找历史结果困难 |
| 15 | 新旧节点类型混乱 | 新用户认知负担 |
